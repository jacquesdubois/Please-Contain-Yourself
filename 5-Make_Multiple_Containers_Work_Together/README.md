...
- [ COMPLETE ] Build your survey server image and tag it with the name 'survey' by running `docker build -t survey .`
...
- [ COMPLETE ] Build your results server image and tag it with the name 'results' by running `docker build -t results .`

- [ COMPLETE ] Pull down a MongoDB image by running `docker pull mongo:latest`
...
- [ COMPLETE ] First, we'll spin up the MongoDB container. Run `docker run -d --name mongo mongo:latest`
...
- [ COMPLETE ] Next let's spin up the other two server images we made, survey and results
...
- [ COMPLETE ] cd into '/survey_server' and run:

`docker run -d -p 8080:8080 -v $(pwd):/src/app --name survey_container survey`

- [ COMPLETE ] cd into '/results_server' and run:

`docker run -d -p 3000:3000 -v $(pwd):/src/app --name results_container results`

- [ COMPLETE ] Now pull up the app by navigating to `localhost:8080` and `localhost:3000` in your browser! You should definitely have your hopes up because I totally didn't give you broken code on purpose!

**(Just kidding, I gave you broken code on purpose)**

- [ COMPLETE ]  As you can see, the app is completly broken. Fortunatly, to get it working, we only have to tweak one line of code in each server. But which one? Run the command `docker logs survey_container` to look for clues inside the container's logs...
...
- [ COMPLETE ] Docker comes installed with 3 ready-to-go networks, to check them out run the command `docker network ls`
...
- [ COMPLETE ] Good to know that they're not floating out in space somewhere, but I'd feel better if we had some visual proof of where they are connected. Run `docker network inspect bridge`
...
- [ COMPLETE ] Open up the 'index.js' for both the survey_server and the results_server

- [ COMPLETE ] On line 8 of both files, change the address string that the server is trying to connect to the database with. Inside the string, change the address 'localhost' to the IPv4Address of the mongo container
...
- [ COMPLETE ] Navigate to `localhost:8080` and `localhost:3000` in separate tabs to check out the app!

- [ COMPLETE ] Fill out the form provided on `localhost:8080` and hit submit to create an entry. Then at `localhost:3000` refresh the page to show all the entries listed!

- [ COMPLETE ] **After you are done with these containers, be sure to remove them (or at least stop them). Nodemon's file watching feature can sometimes take up a chunk of CPU power** Run:
